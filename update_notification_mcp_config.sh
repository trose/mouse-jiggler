#!/bin/bash
# Script to update Qoder's MCP configuration with the new macOS notification MCP path
...
# Update the macos-notification MCP server configuration with the new path
# Use sed to find and replace the existing configuration
sed -i '' '/"macos-notification"/,/^    }/c\
    "macos-notification": {\
      "command": "/Users/trose/src/macos-notification-mcp/venv/bin/python",\
      "args": ["-m", "macos_notification_mcp"],\
